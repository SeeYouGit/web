...
window.miMiniGame = function (exports, Laya) {
		...
		_createClass(MiniFileMgr, null, [{
			...
			value: function isLocalNativeFile(url) {
				for (var i = 0, sz = KGMiniAdapter.nativefiles.length; i < sz; i++) {
					if (url.indexOf(KGMiniAdapter.nativefiles[i]) != -1) return true;
				}
				return false;
			}
		}, {
						...
						MiniFileMgr.fs.getFileInfo({
							...
							success: function success(data) {
								if (isAutoClear && fileUseSize + chaSize + data.size >= totalSize) {
									if (data.size > KGMiniAdapter.minClearSize) KGMiniAdapter.minClearSize = data.size;
	...
	var MiniInput = function () {
		...
		_createClass(MiniInput, null, [{
			...
			value: function wxinputFocus(e) {
				...
				KGMiniAdapter.window.qg.showKeyboard({ defaultValue: _inputTarget.text, maxLength: _inputTarget.maxChars, multiple: _inputTarget.multiline, confirmHold: true, confirmType: _inputTarget["confirmType"] || 'done', success: function success(res) {}, fail: function fail(res) {} });
			...
			value: function hideKeyboard() {
				KGMiniAdapter.window.qg.offKeyboardConfirm();
				KGMiniAdapter.window.qg.offKeyboardInput();
				KGMiniAdapter.window.qg.hideKeyboard({ success: function success(res) {
						console.log('隐藏键盘');
	...
	var KGMiniAdapter = function () {
		function KGMiniAdapter() {
			_classCallCheck(this, KGMiniAdapter);
		}

		_createClass(KGMiniAdapter, null, [{
			key: "getJson",
