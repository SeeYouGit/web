...
window.bdMiniGame = function (exports, Laya) {
		...
		_createClass(MiniFileMgr, null, [{
			...
			value: function isLocalNativeFile(url) {
				for (var i = 0, sz = BMiniAdapter.nativefiles.length; i < sz; i++) {
					if (url.indexOf(BMiniAdapter.nativefiles[i]) != -1) return true;
				}
				return false;
			}
		}, {
						...
						MiniFileMgr.fs.getFileInfo({
							...
							success: function success(data) {
								if (isAutoClear && fileUseSize + chaSize + data.size >= totalSize) {
									if (data.size > BMiniAdapter.minClearSize) BMiniAdapter.minClearSize = data.size;
	...
	var MiniInput = function () {
		...
		_createClass(MiniInput, null, [{
			...
			value: function wxinputFocus(e) {
				...
				BMiniAdapter.window.swan.showKeyboard({ defaultValue: _inputTarget.text, maxLength: _inputTarget.maxChars, multiple: _inputTarget.multiline, confirmHold: true, confirmType: _inputTarget["confirmType"] || 'done', success: function success(res) {}, fail: function fail(res) {} });
			...
			value: function hideKeyboard() {
				BMiniAdapter.window.swan.offKeyboardConfirm();
				BMiniAdapter.window.swan.offKeyboardInput();
				BMiniAdapter.window.swan.hideKeyboard({ success: function success(res) {
						console.log('隐藏键盘');
	...
	var BMiniAdapter = function () {
		function BMiniAdapter() {
			_classCallCheck(this, BMiniAdapter);
		}

		_createClass(BMiniAdapter, null, [{
			key: "getJson",
			...
			value: function init() {
				...
				if (!BMiniAdapter.window.hasOwnProperty("swan")) return;
