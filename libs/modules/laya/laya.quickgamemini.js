...
window.qgMiniGame = function (exports, Laya) {
		...
		_createClass(MiniFileMgr, null, [{
			...
			value: function isLocalNativeFile(url) {
				for (var i = 0, sz = QGMiniAdapter.nativefiles.length; i < sz; i++) {
					if (url.indexOf(QGMiniAdapter.nativefiles[i]) != -1) return true;
				}
				return false;
			}
		}, {
			...
			value: function downLoadFile(fileUrl) {
				...
				var callBack = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
				var encoding = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : "ascii";
						...
						MiniFileMgr.fs.getFileInfo({
							...
							success: function success(data) {
								if (isAutoClear && fileUseSize + chaSize + data.size >= totalSize) {
									if (data.size > QGMiniAdapter.minClearSize) QGMiniAdapter.minClearSize = data.size;
	...
	var MiniInput = function () {
		...
		_createClass(MiniInput, null, [{
			...
			value: function wxinputFocus(e) {
				...
				QGMiniAdapter.window.qg.showKeyboard({ defaultValue: _inputTarget.text, maxLength: _inputTarget.maxChars, multiple: _inputTarget.multiline, confirmHold: true, confirmType: _inputTarget["confirmType"] || 'done', success: function success(res) {}, fail: function fail(res) {} });
			...
			key: "inputEnter",
			value: function inputEnter(isBool) {
				if (isBool) {
					MiniInput.hideKeyboard();
				}
				if (!Laya.Input['inputElement'].target) return;
			...
			value: function hideKeyboard() {
				...
				QGMiniAdapter.window.qg.hideKeyboard({ success: function success(res) {
						console.log('隐藏键盘');
	...
	var QGMiniAdapter = function () {
		function QGMiniAdapter() {
			_classCallCheck(this, QGMiniAdapter);
		}

		_createClass(QGMiniAdapter, null, [{
			key: "getJson",
			...
			value: function init() {
				...
				if (!QGMiniAdapter.window.hasOwnProperty('qg')) return;
