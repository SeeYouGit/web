...
window.vvMiniGame = function (exports, Laya) {
		...
		_createClass(MiniFileMgr, null, [{
			...
			value: function isLocalNativeFile(url) {
				for (var i = 0, sz = VVMiniAdapter.nativefiles.length; i < sz; i++) {
					if (url.indexOf(VVMiniAdapter.nativefiles[i]) != -1) return true;
				}
				return false;
			}
		}, {
			...
			value: function read(filePath) {
				...
				MiniFileMgr.fs.readFile({ uri: fileUrl, encoding: encoding, success: function success(data) {
			...
			value: function downFiles(fileUrl) {
				...
				var downloadTask = MiniFileMgr.wxdown({ url: fileUrl, success: function success(data) {
						if (data.errCode == 0) data.statusCode = 200;
			...
			value: function readFile(filePath) {
				...
				MiniFileMgr.fs.readFile({ uri: filePath, encoding: encoding, success: function success(data) {
				...
				if (fileObj) {
					if (fileObj.readyUrl != readyUrl) {
						MiniFileMgr.fs.getFileInfo({
							...
							success: function success(data) {
								if (data.length) data.size = data.length;
								if (isAutoClear && fileUseSize + chaSize + data.size >= totalSize) {
									if (data.size > VVMiniAdapter.minClearSize) VVMiniAdapter.minClearSize = data.size;
					...
					MiniFileMgr.fs.getFileInfo({
						...
						success: function success(data) {
							...
							MiniFileMgr.fs.copyFile({ srcUri: tempFilePath, dstUri: saveFilePath, success: function success(data2) {
			...
			value: function deleteFile(tempFileName) {
				...
				MiniFileMgr.fs.deleteFile({ uri: deleteFileUrl, success: function success(data) {
						if (tempFileName != "") {
							var saveFilePath = MiniFileMgr.getFileNativePath(tempFileName);
							MiniFileMgr.fs.copyFile({ srcUri: tempFileName, dstUri: saveFilePath, success: function success(data) {}, fail: function fail(data) {
									callBack != null && callBack.runWith([1, data]);
								} });
						}
					}, fail: function fail(data) {} });
			...
			value: function writeFilesList(fileurlkey, filesListStr, isAdd) {
				...
				MiniFileMgr.fs.writeFile({ uri: listFilesPath, encoding: 'utf8', data: filesListStr, success: function success(data) {}, fail: function fail(data) {} });
			...
			value: function existDir(dirPath, callBack) {
				MiniFileMgr.fs.mkdir({ uri: dirPath, success: function success(data) {
	...
	var MiniSound = function (_Laya$EventDispatcher) {
		...
		_createClass(MiniSound, [{
			...
			key: "onCheckComplete",
			value: function onCheckComplete() {
	...
	var MiniInput = function () {
		...
		_createClass(MiniInput, null, [{
			...
			value: function wxinputFocus(e) {
				...
				VVMiniAdapter.window.qg.showKeyboard({ defaultValue: _inputTarget.text, maxLength: _inputTarget.maxChars, multiple: _inputTarget.multiline, confirmHold: true, confirmType: _inputTarget["confirmType"] || 'done', success: function success(res) {}, fail: function fail(res) {} });
			...
			value: function hideKeyboard() {
				VVMiniAdapter.window.qg.offKeyboardConfirm();
				VVMiniAdapter.window.qg.offKeyboardInput();
				VVMiniAdapter.window.qg.hideKeyboard({ success: function success(res) {
						console.log('隐藏键盘');
	...
	var VVMiniAdapter = function () {
		function VVMiniAdapter() {
			_classCallCheck(this, VVMiniAdapter);
		}

		_createClass(VVMiniAdapter, null, [{
			key: "getJson",
			...
			value: function init() {
				...
				if (!VVMiniAdapter.window.hasOwnProperty("qg")) return;
			...
			value: function getUrlEncode(url, type) {
				if (type == "arraybuffer") return "binary";
				return "utf8";
			}
		}, {
			key: "downLoadFile",
