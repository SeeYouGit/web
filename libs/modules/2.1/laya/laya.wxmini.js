...
window.layalib(function (window, document, Laya) {
	...
	var MiniFileMgr = (function () {
		...
		MiniFileMgr.readFile = function (filePath, encoding, callBack, readyUrl, isSaveFile, fileType, isAutoClear) {
			...
			MiniFileMgr.fs.readFile({
				...
				encoding: encoding,
				success: function (data) {
					if (filePath.indexOf("wxfile://") != -1 || filePath.indexOf("http://") != -1 || filePath.indexOf("https://") != -1) {
					...
					success: function (data) {
						...
						MiniFileMgr.fs.copyFile({
							...
							success: function (data2) {
								...
								// console.log("复制文件完成", tempFilePath, saveFilePath);
							},
							fail: function (data) {
								callBack != null && callBack.runWith([1, data]);
							}
		...
		MiniFileMgr.sortOn = function (array, name, options) {
			...
			if (options == 16) return array.sort(function (a, b) {
				return a[name] - b[name];
			});
			if (options == (16 | 2)) return array.sort(function (a, b) {
		...
		__static(MiniFileMgr,
			['fs', function () {
				return this.fs = MiniAdpter.window.wx.getFileSystemManager();
			}, 'wxdown', function () {
				return this.wxdown = MiniAdpter.window.wx.downloadFile;
			}]);
		return MiniFileMgr;
	})()
	...
	var MiniLoader = (function (_super) {
		...
		__proto.load = function (url, type, cache, group, ignoreCache) {
			...
			Laya.loaded_res_pathes = Laya.loaded_res_pathes || {};
			Laya.loaded_res_pathes[url] = true;
			...
			if ((MiniLoader._fileTypeArr.indexOf(urlType) != -1)) {
				...
				if (!MiniFileMgr.getFileInfo(url)) {
					...
					if ((false) && tempurl.indexOf("http://usr/") == -1 && (tempurl.indexOf("http://") != -1 || tempurl.indexOf("https://") != -1) && !MiniAdpter.AutoCacheDownFile) {
						MiniAdpter.EnvConfig.load.call(thisLoader, url, type, cache, group, ignoreCache);
					} else {
						fileObj = MiniFileMgr.getFileInfo(url) || MiniFileMgr.getFileInfo(tempurl);
						if (fileObj) {
							fileObj.encoding = fileObj.encoding == null ? "utf8" : fileObj.encoding;
							var _url = fileObj.url || MiniFileMgr.getFileNativePath(fileObj.md5);
							MiniFileMgr.readFile(_url, encoding, new Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [encoding, url, type, cache, group, ignoreCache, thisLoader]), url);
						} else if (thisLoader.type == "image" || thisLoader.type == "htmlimage") {
							MiniAdpter.EnvConfig.load.call(thisLoader, url, type, cache, group, ignoreCache);
						} else {
							url = URL.formatURL(url);
							if ((url.indexOf("http://") == -1 && url.indexOf("https://") == -1) || MiniFileMgr.isLocalNativeFile(url) || url.indexOf("wxfile://") != -1) {
								MiniFileMgr.readFile(url, encoding, new Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [encoding, url, type, false, group, true, thisLoader]), url);
							} else {
								MiniFileMgr.downFiles(url, encoding, new Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [encoding, url, type, true, group, false, thisLoader]), url, cache);
							}
						}
					}
				} else {
					if (url.endsWith(".json")) {
						...
						var _url = fileObj.url || MiniFileMgr.getFileNativePath(fileObj.md5);
						fileObj.encoding = fileObj.encoding == null ? "utf8" : fileObj.encoding;
						MiniFileMgr.readFile(_url, fileObj.encoding, new Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [encoding, url, type, cache, group, ignoreCache, thisLoader]), url);
						// console.log("使用缓存资源", url, _url);
					}
				}
			}
		}
